...
export default function Converter() {
  ...
  const cnyN = toNumber(cny),
    nubRateN = toNumber(nubRate),
    wiseRateN = toNumber(wiseRate),
    iofN = toNumber(iof),
    markupN = toNumber(markup)
  ...
  function onSalvar() {
    const row = {
      ...
      item: nome,
      cny: cnyN,
      nubank_sem_iof: +nub.semIof.toFixed(2),
      nubank_iof: +nub.iofV.toFixed(2),
      nubank_total: +nub.total.toFixed(2),
      wise_total: +wis.toFixed(2),
      fonte_preco: fonte,
      markup_percent: markupN,
  ...
  function onExport() {
    ...
    a.href = url
    a.download = 'CS2MarketPrice_Historico.csv'
    a.click()
  ...
  return (
    <div className="grid gap-6">
      ...
      <section className="card grid md:grid-cols-2 gap-6 items-center">
        <div className="flex items-center gap-4">
          <img
            ...
            alt="CS2MarketPrice"
            className="w-20 h-20 rounded-lg ring-1 ring-bronze/40"
          />
          <div>
            <h1 className="text-2xl font-display font-bold">CS2MarketPrice</h1>
            <p className="text-sm opacity-80">
              Conversor e precificador de skins (CNY/USD → BRL)
            </p>
          </div>
        </div>
        <div className="text-right md:text-right">
          <span className="text-bronze">v0.1</span>
        </div>
      </section>
      ...
      <section className="card grid sm:grid-cols-2 lg:grid-cols-3 gap-4">
        <div>
          <label className="text-sm opacity-80">Loja/Vendedor</label>
          <input
            value={loja}
            onChange={(e) => setLoja(e.target.value)}
            className="w-full mt-1 bg-night border border-slate rounded-lg px-3 py-2"
            placeholder="Ex.: BUFF/CSFloat"
          />
        </div>
        <div className="sm:col-span-2">
          <label className="text-sm opacity-80">Nome da Skin</label>
          <input
            value={nome}
            onChange={(e) => setNome(e.target.value)}
            className="w-full mt-1 bg-night border border-slate rounded-lg px-3 py-2"
            placeholder="AK Redline (FT)"
          />
        </div>

        <div>
          <label className="text-sm opacity-80">Valor em CNY</label>
          <input
            value={cny}
        ...
        <div>
          <label className="text-sm opacity-80">Taxa Nubank (BRL/CNY)</label>
          <input
            value={nubRate}
            onChange={(e) => setNubRate(e.target.value)}
            className="w-full mt-1 bg-night border border-slate rounded-lg px-3 py-2"
          />
        </div>
        <div>
          <label className="text-sm opacity-80">IOF Nubank (%)</label>
          <input
            value={iof}
        ...
        <div>
          <label className="text-sm opacity-80">Taxa Wise (BRL/CNY)</label>
          <input
            value={wiseRate}
        ...
        <div>
          <label className="text-sm opacity-80">Markup (lucro %)</label>
          <input
            value={markup}
        ...
        <div>
          <label className="text-sm opacity-80">Fonte para precificar</label>
          <select
            value={fonte}
      ...
      <section className="card grid gap-3">
        <div className="grid md:grid-cols-3 gap-4 text-sm">
          <div className="bg-night/60 rounded-lg p-4 border border-slate">
            ...
            <div className="mt-1">
              Sem IOF: <b>{formatBRL(nub.semIof)}</b>
            </div>
            <div>
              IOF: <b>{formatBRL(nub.iofV)}</b>
            </div>
            <div>
              Total: <b className="text-bronze">{formatBRL(nub.total)}</b>
            </div>
          </div>
          ...
          <div className="bg-night/60 rounded-lg p-4 border border-slate">
            ...
            <div>
              Total: <b className="text-bronze">{formatBRL(wis)}</b>
            </div>
          </div>
      ...
      <section className="card">
        <div className="flex items-center gap-2 mb-3">
          <History size={18} /> <b>Histórico recente</b>
        </div>
        {!hist || hist.length === 0 ? (
          <div className="opacity-70 text-sm">Sem registros ainda.</div>
        ) : (
                ...
                {hist.map((r, i) => {
                  ...
                  return (
                    <tr key={i} className="border-t border-slate/40">
                      ...
                      <td>{r.loja}</td>
                      <td className="truncate max-w-[260px]" title={r.item}>
                        {r.item}
                      </td>
